...
The build process is managed through YAML workflows, leveraging GNU COBOL's internal mechanism of reassembling COBOL source code to C before compiling to a native executable.

## How It Works

The pipeline is split into two stages: compilation and execution. It was an attempt to avoid unnecessary recompilation on every run:
...
Even with this separation, each execution requires installing GnuCOBOL and `libcob4` from scratch in the runner environment, which adds an overhead of approximately 16 to 24 seconds per run. This is an inherent limitation of CI/CD environments, not of the program itself.
...
- **Language:** GNU COBOL (reassembled to C at build time)
...
This project demonstrates that COBOL can be integrated into modern DevOps workflows with relatively little effort. The main practical limitation is the absence of a persistent build environment. A Docker-based approach or a pre-built runner image can significantly reduce pipeline execution time.
...
O processo de build é gerenciado por workflows YAML, aproveitando o mecanismo interno do Cobol GNU de remontar o código-fonte COBOL para C antes de compilar para um executável nativo.

## Funcionamento

O pipeline é dividido em duas etapas: compilação e execução. Foi uma tentativa de evitar recompilações desnecessárias a cada execução:
...
Mesmo com essa separação, cada execução exige a instalação do GnuCOBOL e do `libcob4` do zero no ambiente do runner, o que adiciona um overhead de aproximadamente 16 a 24 segundos por execução. Essa é uma limitação inerente aos ambientes de CI/CD e não do programa em si.
